...
def train(config):
    ...
    trainer = JointReDistillTrainer(
        ...
        ppo_config=PPOConfig(
            lr=config["lr"],
            weight_decay=-1,  # TODO replace placeholder
...
def main():
    param_space = {
        "lr": tune.loguniform(10**-4.5, 10**-3),
        "epochs": [1, 2, 3],
        "ppo_clip_coeff": tune.uniform(0.01, 0.2),
        "beta_bc": tune.loguniform(10**-2.5, 1),
        "beta_vf": tune.loguniform(10**-1, 10**1),
    }
